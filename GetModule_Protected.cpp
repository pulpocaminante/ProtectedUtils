...
// Very fast function to get main module base
// x86 ONLY!
LPVOID getprot::GetMainModuleBaseFast_x86()
{
#if defined(_M_X64)
	return NULL;
#elif defined(_M_ARM)
...
LPVOID getprot::GetMainModuleBaseSecure()
{
	...
	const auto base = pPeb->Reserved3[1];
	return base;
}

// GET POINTER TO THREAD ENVIRONMENT BLOCK
LPVOID getprot::GetTEB()
{
	// Get pointer to the TEB
#if defined(_M_X64) // x64
	auto pTeb = reinterpret_cast<PTEB>(__readgsqword(reinterpret_cast<DWORD>(&static_cast<PNT_TIB>(nullptr)->Self)));
...
// GET POINTER TO PROCESS ENVIRONMENT BLOCK
// Slooooow (splitting milliseconds) but *always* works
// 
// NOTE:
// You should use this instead of GetMainModuleBase if you
// need to access the Process Environment Block more than
// once, for efficiencies sake!
LPVOID getprot::GetPPEB()
{
...
LPVOID getprot::GetModuleFunc(LPCWSTR sModuleName, LPCSTR sFuncName)
{
